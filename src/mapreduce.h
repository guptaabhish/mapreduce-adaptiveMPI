...
class MapReduce {
 ...
 public:
  ...
  static __thread MapReduce *mrptr;         // holds a ptr to MR currently being used
  static __thread int instances_now;        // total # of MRs currently instantiated
                                   // grows as created, shrinks as destroyed
  static __thread int instances_ever;       // total # of MRs ever instantiated
                                   // grows as created, never shrinks
  static __thread int mpi_finalize_flag;    // 1 if MR library should finalize MPI
  static __thread uint64_t msize,msizemax;  // current and hi-water memory allocation
  static __thread uint64_t rsize,wsize;     // total read/write bytes for all I/O
  static __thread uint64_t cssize,crsize;   // total send/recv bytes for all comm
  static __thread double commtime;          // total time for all comm
